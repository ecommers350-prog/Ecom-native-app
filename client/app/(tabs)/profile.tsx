      ...
      <ScrollView className='flex-1 px-4'
        ...
        {!user ? (
          ...
          <View className='items-center w-full'>
            ...
            <Text className='text-black font-bold text-xl mb-2'>Guest User</Text>
            <Text className='text-black/20 text-base mb-8 text-center w-3/4 px-4 '>Log in to view your profile, orders, and addresses.</Text>
            <TouchableOpacity className='bg-black w-3/5 py-3 rounded-full items-center shadow-lg' onPress={() => router.push('/sign-in')}>
          ...
          <>
            ...
            <View className="items-center mb-8">
                ...
                <Text className="text-x1 font-bold text-primary">{user.firstName + " " + user.lastName}</Text>
                <Text className="text-secondary text-sm">{user.emailAddresses[0].emailAddress}</Text>

                {/* Admin Panel Button if user is admin */}
                {user.publicMetadata?.role === "admin" && (
                  <TouchableOpacity onPress={() => router.push('/admin')} className='mt-4 bg-black px-6 py-2 rounded-full'>
            ...
            <View>
              {PROFILE_MENU.map((item, index) => (
                ...
                onPress={()=> router.push(item.route as any)}>
                  ...
                  <Text className='flex-1 text-black font-medium'>{item.title}</Text>
