...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/product/${product._id}`} asChild>
      <TouchableOpacity className="w-[48%] bg-white rounded-lg overflow-hidden">
        <View className="relative h-56 w-full bg-gray-100">
          ...
          {/* Faviourte icon */}
          <TouchableOpacity className="top-2 absolute right-2 z-10 p-2 bg-white rounded-full shadow-sm" onPress={(e)=> {e.stopPropagation(); toggleWishlist(product)}}>
            <Ionicons name={isLiked ? 'heart' : 'heart-outline'} size={20} color={isLiked ? COLORS.accent : COLORS.primary} />
          </TouchableOpacity>

          {/* is Featured */}
          {product.isFeatured && (
            <View className="top-2 left-2 bg-black px-2 py-1 rounded absolute">
